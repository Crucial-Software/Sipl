...
import AllAmcListItem from '../components/AllAmcListItem';
...
const AllAmc = () => {
  ...
  const getWork = async (locid) => {
    ...
    await fetch(`${API_BASE}/app/work/listamc`, {
      ...
      .catch((error) => {
        setLoading(false);
        console.error('WORK DPR There was an error!', error);
  ...
  const searchByDate = () => {
    ...
    console.log("AllAmc FromDate: " + moment(fromDate).format("DD-MMM-YYYY") + " ToDate: " + moment(toDate).format("DD-MMM-YYYY"));
